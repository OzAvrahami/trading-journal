...
export function PositionSummary({ item, actions, quote = null, quoteStatus = null }) {
  ...
  const liveQuote = quote && Number.isFinite(Number(quote.price)) && Number(quote.price) > 0 ? quote : null;
  return <Card density="compact" as="article">
    ...
    {liveQuote && <section className="mt-4 rounded-md bg-information-soft p-3" aria-label={t('investments.liveMarketData')}>
      ...
      <dl className="mt-3 grid grid-cols-3 gap-3">
        <Metric label={t('investments.livePrice')} value={formatNumber(liveQuote.price, { minimumFractionDigits: 2, maximumFractionDigits: 4 })} />
        <Metric label={t('investments.dayChange')} value={<ValueIndicator value={liveQuote.change}>{liveNumber(liveQuote.change)}</ValueIndicator>} />
    ...
    {!liveQuote && quoteStatus === 'missing' && <p className="mt-4 rounded-md bg-surface-sunken p-3 text-xs text-muted" role="status">{t('investments.liveQuoteMissing')}</p>}
    ...
    <p className="mt-3 text-xs text-muted">{t('portfolio.priceDate')}: <span dir="ltr">{formatDate(item.latestPriceDate)}</span></p>
