  ...
  return (
    <form onSubmit={handleSubmit(submit)} className="space-y-4" noValidate>
      <div className="grid gap-4 adaptive:grid-cols-2">
        <Field label={t('common.rule')} required error={errors.ruleId?.message}>
          {(fieldProps) => (
            <Select data-autofocus {...fieldProps} {...register('ruleId', { required: t('rules.chooseRule') })}>
              <option value="">{t('common.rule')}</option>
              {ruleOptions.map((rule) => <option key={rule.id} value={rule.id}>{rule.name}{rule.isActive === false ? ` (${t('status.inactive')})` : ''}</option>)}
            </Select>
          )}
        </Field>
        <Field label={t('rules.checkDate')} required error={errors.checkDate?.message}>
          {(fieldProps) => <Input type="date" dir="ltr" {...fieldProps} {...register('checkDate', { required: t('rules.chooseCheckDate') })} />}
        </Field>
      </div>

      {selectedRule && (
        <p className="rounded-md border border-information bg-information-soft px-3 py-2 text-xs text-secondary">
          {t(`rules.scopeHelp.${selectedRule.scope}`)}{' '}{t('common.scope')}: {scopeLabel(selectedRule.scope)}.
        </p>
      )}

      <fieldset>
        <legend className="label">{t('common.outcome')} <span className="text-negative" aria-hidden="true">*</span></legend>
        <div className="grid gap-2 adaptive:grid-cols-3">
          {RULE_OUTCOMES.map((outcome) => (
            <label key={outcome.value} className="flex min-h-11 cursor-pointer items-center gap-2 rounded-md border border-default bg-surface-raised px-3 text-sm text-secondary has-[:checked]:border-action has-[:checked]:bg-action-soft has-[:checked]:font-semibold has-[:checked]:text-primary">
              <input type="radio" value={outcome.value} className="h-4 w-4 accent-action" {...register('outcome', { required: t('rules.chooseOutcome') })} />
      ...
      <Field label={t('common.notes')} helpText={t('routes.rules.description')} error={errors.notes?.message}>
        {(fieldProps) => (
          <textarea rows={3} maxLength={5000} className="input min-h-20 resize-y" {...fieldProps} {...register('notes', { maxLength: { value: 5000, message: t('validation.maxLength', { field: t('common.notes'), max: 5000 }) } })} />
        )}
      </Field>

      <fieldset className="rounded-lg border border-default bg-surface-raised p-3">
        <legend className="px-1 text-xs font-semibold text-secondary">{t('rules.linkedTrade')}</legend>
        <label className="flex min-h-11 items-center gap-2 rounded-md border border-default bg-surface px-3">
          ...
          <input type="search" value={tradeSearch} onChange={(event) => setTradeSearch(event.target.value)} placeholder={t('journal.searchBySymbol')} className="min-w-0 flex-1 border-0 bg-transparent text-sm text-primary outline-none placeholder:text-muted" />
        </label>
        <div className="mt-3 max-h-44 space-y-1 overflow-y-auto" role="radiogroup" aria-label={t('journal.availableTrades')}>
          <label className="flex min-h-11 cursor-pointer items-center gap-3 rounded-md px-2 text-sm text-secondary hover:bg-surface-sunken">
            <input type="radio" name="linked-trade" checked={!tradeId} onChange={() => setTradeId('')} className="h-4 w-4 accent-action" /> {t('rules.noLinkedTrade')}
          </label>
          {tradesQuery.isError && <p className="px-2 py-2 text-sm text-negative" role="alert">{t('rules.tradesLoadFailed')}</p>}
          {trades.map((trade) => {
            ...
            return (
              <label key={trade.id} className="flex min-h-11 cursor-pointer items-start gap-3 rounded-md px-2 py-2 text-sm text-secondary hover:bg-surface-sunken">
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="ms-2">{t(`status.${trade.status}`)}</span>
                  <span className="mt-0.5 block truncate text-xs text-muted"><span dir="ltr">{formatDateKey(normalizeDateKey(trade.entryDatetime))}</span>{account ? <>{' · '}<bdi>{account}</bdi></> : null}</span>
                </span>
              </label>
            );
          })}
      ...
      <fieldset className="rounded-lg border border-default bg-surface-raised p-3">
        <legend className="px-1 text-xs font-semibold text-secondary">{t('rules.linkedJournal')}</legend>
        <label className="flex min-h-11 items-center gap-2 rounded-md border border-default bg-surface px-3">
          <MagnifyingGlass size={16} className="text-muted" aria-hidden="true" />
          <span className="sr-only">{t('rules.searchJournal')}</span>
          <input type="search" value={journalSearch} onChange={(event) => setJournalSearch(event.target.value)} placeholder={t('rules.searchJournalPlaceholder')} className="min-w-0 flex-1 border-0 bg-transparent text-sm text-primary outline-none placeholder:text-muted" />
        </label>
        <div className="mt-3 max-h-44 space-y-1 overflow-y-auto" role="radiogroup" aria-label={t('rules.availableJournal')}>
          <label className="flex min-h-11 cursor-pointer items-center gap-3 rounded-md px-2 text-sm text-secondary hover:bg-surface-sunken">
            <input type="radio" name="linked-journal" checked={!journalEntryId} onChange={() => setJournalEntryId('')} className="h-4 w-4 accent-action" /> {t('rules.noLinkedJournal')}
          </label>
          {journalQuery.isError && <p className="px-2 py-2 text-sm text-negative" role="alert">{t('rules.journalLoadFailed')}</p>}
          {entries.map((entry) => (
            <label key={entry.id} className="flex min-h-11 cursor-pointer items-start gap-3 rounded-md px-2 py-2 text-sm text-secondary hover:bg-surface-sunken">
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="mt-0.5 block text-xs text-muted"><span>{t(`status.${entry.entryType}`)}</span> · <span dir="ltr">{formatDateKey(entry.entryDate)}</span> · {t(`status.${entry.isComplete ? 'complete' : 'incomplete'}`)}</span>
              </span>
            </label>
          ))}
        </div>
      ...
      <Button type="submit" variant="primary" size="mobile" className="w-full" loading={loading}>
        {isEdit ? t('rules.saveCheck') : t('rules.recordCheckSubmit')}
      </Button>
    </form>
  );
}
