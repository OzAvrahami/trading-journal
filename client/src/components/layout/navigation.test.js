...
describe('Trading navigation', () => {
  it('places managed Strategies between Trades and Daily Review without removing existing routes', () => {
    const trading = navigationGroups.find((group) => group.label === 'Trading');
    expect(trading.items.map((item) => item.label)).toEqual(['Dashboard', 'Trades', 'Strategies & Setups', 'Daily Review']);
  ...
  it('keeps primary mobile navigation bounded and exposes Strategies through More', () => {
    expect(mobileNavigationItems).toHaveLength(3);
    expect(mobileMoreItems.find((item) => item.to === '/strategies')).toMatchObject({ label: 'Strategies & Setups' });
  });
});
