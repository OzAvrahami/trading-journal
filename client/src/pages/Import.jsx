  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      {step === 'preview' && parseStats && <div className="space-y-6">
        ...
        <div className="flex items-center gap-3"><button type="button" onClick={handleCommit} disabled={committing || Boolean(duplicateRun) || parseStats.uniqueInFile === 0 || !accountId} className="btn-primary flex min-h-11 items-center gap-2 disabled:cursor-not-allowed disabled:opacity-50">{committing ? <><Spinner className="h-4 w-4" label={t('importPage.importing')} /> {t('importPage.importing')}</> : t('importPage.importCount', { count: parseStats.uniqueInFile })}</button><button type="button" onClick={handleReset} className="btn-secondary min-h-11">{t('importPage.startOver')}</button></div>
      </div>}
